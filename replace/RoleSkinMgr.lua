...
function this:GetSkinsRet(proto)
   --我的修改部分
    --进战斗自动替换进阶皮为付费皮
    local tab = FileUtil.LoadByPath("skinMgr.txt", 0)
    local mergedSkins = tab or { skinUsedObj = {} }

    for _, obj in ipairs(mergedSkins.skinUsedObj) do
        -- 替换皮肤
        RoleMgr:CardUpdate(obj)
    end
...
function this:UseSkin(_cid, _skin, _skin_a, _isL2d, _isL2d_a)
    ...
    --我的修改部分
    --判断没有皮肤就不发送请求直接模拟本地,并写入文件以便登录和读取模型时使用
    local canUseFlag = self.datas[_cid][_skin]:GetCanUse()
    LuaLog("canUseFlag")
    LuaLog(canUseFlag)
    if (canUseFlag) then
        NetMgr.net:Send(proto)
    else
        local _proto = {["store_exp"] = _cid,
            ["cards"] = {
                [1] = {
                    ["skin"] = _skin,
                    ["skin_a"] = _skin_a,
        ...
        for i, obj in ipairs(mergedSkins.skinUsedObj) do
            if obj.cards[1].cfgid == _proto.cards[1].cfgid then
                objectExists = true
                objectIndex = i
                break
            end
        ...
        if not objectExists then
            table.insert(mergedSkins.skinUsedObj, _proto)
        else
            -- 更新已存在的对象
            mergedSkins.skinUsedObj[objectIndex] = _proto
        end

        -- 换肤
        RoleMgr:CardUpdate(_proto)

        -- 保存合并后的数据
        FileUtil.SaveToFile("skinMgr.txt", mergedSkins, 0)
    end
    --我的修改完毕
end
