-- OPENDEBUG()
--我的修改部分
local skinConfig = require("cfgCfgSkinInfo")
--我的修改完毕
-- 冒泡排序(稳定)
...
function FightMgrBase:LoadData(teamID, data, typ, tCommanderSkill)
    ...
    -- 替换皮肤
    for _, pm in ipairs(data) do
        local skinData = skinMap[pm.data.id]
        if skinData then
            pm.data.model = skinData.skin
            if skinData.skin_a then
                pm.data.modelA = skinData.skin_a
            end

            -- 如果需要日志输出，可以取消下面的注释
            -- LuaLog("Skin replaced for id: " .. pm.data.id)
            -- LuaLog("New model: " .. pm.data.model)
            -- if skinData.skin_a then
            --     LuaLog("New modelA: " .. pm.data.modelA)
            -- end
        end
    end
