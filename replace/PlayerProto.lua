...
function PlayerProto:SetNewPanel(_panels, _setting, _random, _using, _cb)
   ...
   --我的修改部分
    local proto = {panels = _panels,	setting=_setting,random=_random,using = _using}
    FileUtil.SaveToFile("kanban.txt", proto,0)
    self:GetNewPanelRet(proto)
end

function PlayerProto:GetNewPanelRet(proto)
    local cache = FileUtil.LoadByPath("kanban.txt", 0)
    local dataToUse = proto

    if cache and cache ~= "" then
        if not self:areTablesEqual(cache, proto) then
            -- 如果不同，使用 cache
            dataToUse = cache
        end
    end
    ...
    CRoleDisplayMgr:GetNewPanelRet(dataToUse)
...
-- 辅助函数：比较两个表是否相等
function PlayerProto:areTablesEqual(t1, t2)
    ...
    for k, v in pairs(t2) do
        if not self:areTablesEqual(v, t1[k]) then
            return false
        end
    end
