...
function FightProto:EnterFightDuplicate(data)
	g_FightProto_id = data.nDuplicateID
	if data.nDuplicateID == 5101 then
		--Tips.ShowTips(data.nDuplicateID)
		Tips.ShowTips("成功为该好友刷助战25次 qwq")
		for i=1,25 do
			local proto3 = {"FightProtocol:EnterFightDuplicate", data};
			NetMgr.net:Send(proto3);

			local proto2 = {"FightProtocol:Quit", { bIsQuit = true}}
    		NetMgr.net:Send(proto2);

			--NetMgr.net:Send(proto4);
