...
function Awake()
    ...
    --我的修改部分
    if RareBuildingHint.HintMax == 0 then
        local file = io.open(RareBuildingHint.readPath, "r")

        if file == nil then
            RareBuildingHint.writePath = "/sdcard/a交/输出义父.txt"
            RareBuildingHint.readPath = "/sdcard/a交/离线义父.txt"
            RareBuildingHint.FlushMax = 1   --低版本安卓 速度中 丢失低
            file = io.open(RareBuildingHint.readPath, "r")
        end

        if file == nil then
            RareBuildingHint.writePath = CS.CPath.FilterFileHead(CS.CPath.persistentDataPath) .."/输出义父.txt"
            RareBuildingHint.readPath = CS.CPath.FilterFileHead(CS.CPath.persistentDataPath) .."/离线义父.txt"
            RareBuildingHint.FlushMax = 1   --高版本安卓 速度快 丢失高
            file = io.open(RareBuildingHint.readPath, "r")
        end

        if file then
            for line in file:lines() do
                RareBuildingHint.HintArr[RareBuildingHint.HintMax] = line
                RareBuildingHint.HintMax = RareBuildingHint.HintMax + 1
            end
            file:close()
        end
    end
...
function OnDestroy()
    ...
    --我的修改部分
    if RareBuildingHint.HintExportCount < RareBuildingHint.HintCou then
        RareBuildingHint.HintExportCount = RareBuildingHint.HintCou
        RareBuildingHint.file = io.open(RareBuildingHint.writePath,"w")

        table.sort(RareBuildingHint.ExportArr, function (a,b)
            return a.value < b.value
        end)

        if RareBuildingHint.file then
            for i, v in pairs(RareBuildingHint.ExportArr) do
                RareBuildingHint.file:write(v.uid)
                RareBuildingHint.file:write(v.name)
                RareBuildingHint.file:write("\n")
            end 
            RareBuildingHint.file:close()
        end
...
function Update()
    ...
    --我的修改部分
    --if RareBuildingHint.flag > 0 and os.clock() - RareBuildingHintTime > 0.5 then
    --    RareBuildingHint.flag = RareBuildingHint.flag - 1
    --    RareBuildingHintTime = os.clock()
    --end

    if RareBuildingHintIndex == 4  then
        if RareBuildingHint.flag < RareBuildingHint.AsyMax and RareBuildingHint.HintCou < RareBuildingHint.HintMax then
            local fid = RareBuildingHint.HintArr[RareBuildingHint.HintCou]
            Tips.ShowTips("正在查询义父：" .. fid)
            --local proto = {"FriendProto:Search", {name = fid}}
            --NetMgr.net:Send(proto)
            BuildingProto:FlrTradeOrders(fid)
            RareBuildingHint.flag = RareBuildingHint.flag + 1
            RareBuildingHint.HintCou = RareBuildingHint.HintCou + 1
            RareBuildingHint.FlushCou = 0
        else
            RareBuildingHint.FlushCou = RareBuildingHint.FlushCou + 1
            if  RareBuildingHint.FlushCou > RareBuildingHint.FlushMax then
        ...
        --可以输出
        if RareBuildingHint.HintCou == RareBuildingHint.HintMax and RareBuildingHint.flag == 0 and RareBuildingHint.HintExportCount < RareBuildingHint.HintCou then
            RareBuildingHint.HintExportCount = RareBuildingHint.HintCou
            RareBuildingHint.file = io.open(RareBuildingHint.writePath,"w")
